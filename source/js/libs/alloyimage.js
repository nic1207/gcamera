...
;(function(Ps){
    ...
    window[Ps].prototype = {
        ...
        //组合效果
        ps: function(effect,a){
            var fun = P.reflectEasy(effect);
            var _this = this;

            _this = fun.call(_this,a);
            ...
            getFun: function(fun){
                var Effects = {
                    ...
                    sketch: function(){//素描
                        ...
                        /*
                        return this.add(
                            _this.act("反色").act("高斯模糊",7), "颜色减淡"
                        ).act("toGray").act("锐化",1);
                        */
                        return this.act("toGray").add(
                          _this.act("toGray").act("反色").act("快速高斯模糊",7),"颜色减淡"
                        );
                    },
                    ...
                    purpleStyle: function(){
                        ...
                        /*
                        return this.add(
                            _this.act("高斯模糊",3),"正片叠底","RB"
                        );
                        */
                        //return this.act("色相/饱和度调节",36,47,8,true).act("暗角", 6, 150);;
                        var m = this.clone().act("色相/饱和度调节",27,25,15,true);
                        return this.add(
                           m, "叠加"
                        );
                    },
                    soften: function(light,blur){
                      if(!blur)blur=3;
                      if(!light)light=4;
                      //console.log("blur=",blur,"light=",light);
                      if(blur>8)
                        blur=8;
                      if(blur<3)
                        blur=3;
                      ...
                      if(light<4)
                        ...
                        */
                        return this.act("快速高斯模糊",blur).act("sharp",2).act("亮度",light,2);
                    },
                    vintage: function(){//复古
                        /*
                        var _this = this.clone();
                        return this.act("toGray").add(
                            window[Ps](this.canvas.width,this.canvas.height,"#808080").act("添加杂色").act("快速高斯模糊",4).act("色相/饱和度调节",32,19,0,true),"叠加"
                        );
                        */
                       //return this.act("sepia").act("noise",10);
                       return this.act("色相/饱和度调节",32,20,0,true).act("noise",12);
                    },
                    gray: function(){//黑白
                        return this.act("toGray");
                    },
                    lomo: function(){//仿lomo
                    ...
                    //夢幻
                    dream: function(){
                        var m = this.clone().act("快速高斯模糊",15);              
                        return this.add(
                            m, "疊加"
                        ); 
                    },
                    ...
                    //甜蜜
                    sweet: function(){
                       var m = this.clone().act("色相/饱和度调节",0,50,20,true).act("曲线",[0,70,142,194,255],[0,174,210,237,255]);
                        return this.add(
                            m, "叠加"
                        );
                    },
                    ...
                    //森女
                    forest: function(){
                        var m = this.clone().act("色相/饱和度调节",90,10,10,true);
                        return this.add(
                           m, "叠加"
                        );
                    }
...
;(function(Ps){
    ...
    window[Ps].module("Filter.fastgaussBlur",function(P){
        ...
        var M = {
            ...
            process: function(imgData,radius, sigma) {
                ...
                */
                ...
                imgData = stackBlurCanvasRGB(imgData, 0, 0, width, height, radius);
                return imgData;
              }
            };
...
;(function(Ps){
    ...
    window[Ps].module("Filter.glow",function(P){

        var M = {
            process: function(imgData,amount, sigma) {
                var pixes = imgData.data;
                var width = imgData.width;
                var height = imgData.height;
                var tmpData = [];
            
                tmpData = stackBlurCanvasRGB(imgData, 0, 0, width, height, 8);
                
                for (i=0;i<width*height*4;i+=4) {
                r = imgData[i]   + tmpData[i]   * amount;
                g = imgData[i+1] + tmpData[i+1] * amount;
                b = imgData[i+2] + tmpData[i+2] * amount;
                if (r > 255) r = 255;
                if (g > 255) g = 255;
                if (b > 255) b = 255;
                imgData[i]   = 255;
